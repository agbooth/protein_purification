...
- (void)layoutSubviewsForInterfaceOrientation:(UIInterfaceOrientation)theOrientation withAnimation:(BOOL)animate
{
    ...
    // Fix for bug in iOS8
    if ([self isLandscape] && (height > width))
    {
        float temp = height;
        height = width;
        width = temp;
	...
	if ([self isVertical]) {
		...
		if (masterFirst) {
            ...
            //NSLog(@"Master rectangle is %.0f, %.0f, %.0f, %.0f",masterRect.origin.x,masterRect.origin.y,masterRect.size.width,masterRect.size.height);
			
			newFrame.origin.x += newFrame.size.width;
			newFrame.size.width = _splitWidth;
			dividerRect = newFrame;
            
            //NSLog(@"Divider rectangle is %.0f, %.0f, %.0f, %.0f",dividerRect.origin.x,dividerRect.origin.y,dividerRect.size.width,dividerRect.size.height);
			
			newFrame.origin.x += newFrame.size.width;
			newFrame.size.width = width - newFrame.origin.x;
			detailRect = newFrame;
            
            //NSLog(@"Detail rectangle is %.0f, %.0f, %.0f, %.0f",detailRect.origin.x,detailRect.origin.y,detailRect.size.width,detailRect.size.height);
