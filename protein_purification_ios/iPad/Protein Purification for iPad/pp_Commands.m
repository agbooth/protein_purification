...
- (void) showProgress
{
  ...
  /*
        ...
        CallBack callback = new CallBack() {
            public void executeBoolean( boolean bool1) {
                if (bool1) abandonScheme();
                else goHome();
            }
        };
        
        
        new YesNoDialog(WindowText,MessageText, callback );
        return;
    }
    
    //no - show the result of this step
    String title = messages.commands_title2(step);
    new ProgressDialog(title, step, app.getAccount());
   */
   ...
   /*
    ...
    [panel setBackgroundColor:[UIColor redColor]];
...
- (void) finishAmmoniumSulfate: (int) choice
{
    ...
    if (choice == 1)   // precipitated material chosen
    {
...
{
//    NSLog(@"Abandon ship button index is %d", buttonIndex);

//    if ((alertView == alert1) && (buttonIndex == 0))
//    {
 //       // Abandon scheme
//        [self abandonScheme];
//    }
